...
async def get_current_user(authorization: str=Depends(oauth2_scheme)):
    ...
    try:
        ...
        session_key: str = payload.get('srandkey')
        ...
        with Connection() as db:
                result = db.query_dict('SELECT * FROM auth_session WHERE session_key = %s AND expire_date > NOW() LIMIT 1', [session_key])
                if not result:
...
async def login_for_access_token(response: Response, username: str=Form(...), password: str=Form(...)):
    ...
    access_token = create_access_token(
        data={'sub': user['username'], 'srandkey': session_key}, expires_delta=access_token_expires
    )
    try:
        with Connection() as db:
            db.execute('INSERT INTO auth_session (session_key, access_token, expire_date) VALUES (%s, %s, NOW() + INTERVAL %s MINUTE)', [session_key, access_token, ACCESS_TOKEN_EXPIRE_MINUTES])
    except Exception:
...
async def refresh_for_access_token(response: Response, request: Request, user=Depends(get_current_user)):
    ...
    try:
        ...
        new_access_token = create_access_token(
            data={'sub': user['username'], 'srandkey': new_session_key}, expires_delta=access_token_expires
        )
        ...
        with Connection() as db:
            ...
            # ! insere novo Token
            db.execute('INSERT INTO auth_session (session_key, access_token, expire_date) VALUES (%s, %s, NOW() + INTERVAL %s MINUTE)', [new_session_key, new_access_token, ACCESS_TOKEN_EXPIRE_MINUTES])
    except JWTError:
        raise credentials_exception
